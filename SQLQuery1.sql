...
select TOP 10 product_id, sum(profit) as profit from df_orders 
group by product_id 
...
-- Here we are using with clause (also known as CTE Common Table Expression/Sub-Query Factoring)
with my_cte as (
...
with cte as (
select category, format(order_date,'yyyyMM') as order_year_month, sum(sale_price) as total_sales
from df_orders
